...
void behaviourSelector(byte type)
{
  ...
  switch(type){
    ...
    case 4:
      positions[0] = sin(la*y + la*h1) + sin(la*x + la*h1) + ld*cos(la*z + la*h2);
      positions[1] = sin(lb*z + lb*h1) + sin(lb*y + lb*h1) + le*cos(lb*x + lb*h2);
      positions[2] = sin(lc*x + lc*h1) + sin(lc*z + lc*h1) + lf*cos(lc*y + lc*h2);
      break;
